...
export default function UserProfile() {
  ...
  const fetchUserInfo = async () => {
    try {
      ...
      const user = Array.isArray(res) ? res.find(user => user.email === userInfo?.email) : res.email === userInfo?.email ? res : null;
      if (user) {
        setFormData(user);
        ...
        <div>
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() => isEditing && handleInputChange("is_sponser", true)}
              disabled={!isEditing}
            >
              Yes
            </button>
